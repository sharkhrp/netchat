        ...
        for readable_object in readable:
            if readable_object == server_command.stdout:
                ...
                if sent_flag == False:
                    print('aaa')
                    clear_previous_line()
                    # Received messages in green
                    print(
                        f'{Fore.RESET}[ {Fore.GREEN}{Style.BRIGHT}MESSAGE{Fore.RESET} ] : {yellow_color}{line}{Fore.RESET}')

                    # Print the prompt immediately in yellow
                    print(f'{Fore.RESET}[ {Fore.MAGENTA}{Style.BRIGHT}SEND{Fore.RESET} ] : {yellow_color}', end='',
                          flush=True)
...
try:
    ...
    print(
        f"{Fore.RESET}[ {Fore.LIGHTRED_EX}{Style.BRIGHT}INFO{Fore.RESET} ] : {Fore.LIGHTGREEN_EX}the connection has been stopped, sir{Fore.RESET}")
    exit_flag = True
